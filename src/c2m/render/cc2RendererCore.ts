...
function renderThinWalls(ts: CC2Tileset, tile: TileSpecObjJson): RgbaImage {
  let out = ts.drawEmpty();
...
function renderNoSign(ts: CC2Tileset): RgbaImage {
  return ts.draw(14, 5);
}

function renderSwivelOverlay(ts: CC2Tileset, terrainName: string): RgbaImage | null {
...
function renderDirectionalBlock(ts: CC2Tileset, tile: TileSpecObjJson): RgbaImage {
...
function renderRailroadTrack(ts: CC2Tileset, tile: TileSpecObjJson): RgbaImage {
  ...
  for (let i = 0; i < 6; i++) if (tracks[i]) base = ts.merge(ts.draw(woodX + i, 30), base);
...
function renderLogicGate(ts: CC2Tileset, tile: TileSpecObjJson): RgbaImage {
  ...
  if (m && m.kind === "LOGIC") {
    if (m.gate === "COUNTER") value = 0x1e + (m.counterValue ?? 0);
    else {
  ...
  if (value >= 0 && value < 24) {
    ...
    return ts.merge(ts.draw(x, y), base);
  }

  if (value >= 30 && value < 40) {
    base = ts.merge(ts.draw(14, 26), base);
    const counter = ts.drawCounter(value - 30, 3);
    return ts.mergeWithOffset(counter, base, T / 8, 0);
  }

  if (value >= 64 && value < 68) {
    return ts.merge(ts.draw((value % 16) + 8, 21), base);
  }

  return base;
}

function renderTerrain(ts: CC2Tileset, tile: TileSpecObjJson): RgbaImage {
        ...
        ? m.style === "GREEN"
          ...
          : m.style === "PINK"
            ...
            : m.style === "YELLOW"
              ...
              : 3
        : 0;
    return name === "CUSTOM_FLOOR" ? ts.draw(8 + styleIndex, 4) : ts.draw(12 + styleIndex, 4);
  }
  ...
  if (name === "LETTER_TILE") {
    ...
    if (m && m.kind === "LETTER_SYMBOL") {
      ...
      if (arrowCode !== null) base = ts.merge(ts.drawSmall(arrowCode, 62), base, "CENTER");
      else if (sym.length === 1) {
        const code = sym.charCodeAt(0);
        if (code >= 0x20 && code <= 0x5f) {
          const letterX = code % 32;
          const letterY = Math.floor(code / 32) - 1;
          base = ts.merge(ts.drawSmall(letterX, letterY), base, "CENTER");
        }
      }
    }
    return base;
  ...
  if (name === "TURTLE") return ts.merge(ts.draw(13, 12), ts.draw(12, 24));
  ...
  const staticXY: Record<string, { x: number; y: number }> = {
    WALL: { x: 1, y: 2 },
    ICE: { x: 10, y: 1 },
    ICE_CORNER_NE: { x: 13, y: 1 },
    ICE_CORNER_NW: { x: 14, y: 1 },
    ICE_CORNER_SE: { x: 11, y: 1 },
    ICE_CORNER_SW: { x: 12, y: 1 },
    WATER: { x: 12, y: 24 },
    FIRE: { x: 12, y: 29 },
    FORCE_N: { x: 0, y: 19 },
    FORCE_E: { x: 2, y: 19 },
...
function renderSob(ts: CC2Tileset, tile: TileSpecObjJson): RgbaImage {
  const name = tile.tile;
  ...
  if (name === "CHERRY_BOMB") return ts.merge(ts.drawSmall(14, 8), ts.draw(5, 4), "TOP_RIGHT");
  if (name === "GREEN_BOMB") return ts.merge(ts.drawSmall(14, 8), ts.draw(6, 4), "TOP_RIGHT");
...
function renderMob(
  ...
  if (name === "MIRROR_CHIP" || name === "MIRROR_MELINDA") {
    ...
    return ts.merge(base, mirror);
  }

  if (name === "DIRT_BLOCK") return ts.draw(8 + (ctx.hideableHidden ? 0 : 1), 1);
  if (name === "ICE_BLOCK") return ts.draw(10 + (ctx.hideableHidden ? 0 : 1), 2);
  ...
  if (name === "DIRECTIONAL_BLOCK") return renderDirectionalBlock(ts, tile);
...
export class CC2RendererCore {
  public constructor(private readonly tileset: CC2Tileset) {}

  public renderMap(map: MapJson): RgbaImage {
    const out = createImage(map.width * T, map.height * T, [0, 0, 0, 255]);
  ...
  public renderLevelDoc(doc: C2mJsonV1): RgbaImage {
    if (!doc.map) throw new Error("Level has no map");
    return this.renderMap(doc.map);
  }
}
